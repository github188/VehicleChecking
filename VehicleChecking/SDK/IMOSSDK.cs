 ...
 */
    ...
    [StructLayout(LayoutKind.Sequential)]
    public struct VEHICLE_BLACKLIST_S
    {
        //黑名单编号
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_CODE_LEN)]
        public byte[] szBlacklistCode;
        //是否模糊匹配，BOOL_TRUE: 模糊匹配，BOOL_FALSE: 精确匹配，目前仅支持精确匹配，填BOOL_FALSE
        public UInt32 bFuzzyMatch;
        //号牌号码
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_CODE_LEN)]
        public byte[] szLicensePlate;
        //号牌颜色，参考数据字典
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_DICTIONARY_KEY_LEN)]
        public byte[] szLicensePlateColor;
        //号牌种类，参考数据字典，可以不填，默认为空，表示所有种类
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_DICTIONARY_KEY_LEN)]
        public byte[]  szLicensePlateType;
        //录入部门编码，可以为空
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_CODE_LEN)]
        public byte[] szDispositionDept;
        //录入人编码，可以为空
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_CODE_LEN)]
        public byte[] szDispositionUser;
        //黑名单来源，参考BLACKLIST_SRC_E
        public UInt32 ulSource;
        //布控优先级，取值范围为1-63，1表示最高，默认为3
        public UInt32 ulPriority ;
        //黑名单原因，暂不使用，填空
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_DICTIONARY_KEY_LEN)]
        public byte[] szDispositionType;
        //添加黑名单的时刻，查询返回，入参可不填
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_TIME_LEN)]
        public byte[] szAddTime;
        //布控开始时间，可以为空
        [MarshalAs(UnmanagedType.ByValArray,SizeConst=IMOSSDK.IMOS_TIME_LEN)]
        public byte[] szBeginTime;
        //布控结束时间，可以为空
    ...
    public class IMOSSDK
    {
        ...
        public const int IMOS_DICTIONARY_KEY_LEN = 8;
